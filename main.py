...
def loop(processed_lines, prot_dict, sphere_point):
    ...
    # Process all the slices of the current line
    process_slices_of_line(dist_ca_to_plane, line, nb_steps)
    # print(line["slice_hydro"])
    line_average_hydro = np.true_divide(np.sum(line["slice_hydro"]), nb_steps)
...
def process_slices_of_line(dist_ca_to_plane, line, nb_steps):
    ...
    for step in range(nb_steps):
        ...
        # Slide the plane along the line. Step = 1 angström
...
if __name__ == '__main__':
    ...
    func = partial(loop, processed_lines, prot_dict)
    ...
    best_line = protein.get_best_line(processed_lines)

    print("Best line: \n\t", "Point of the sphere: ", best_line[0] + center_of_mass, "\n\tAverage hydrophobicity: {:.4f}".format(best_line[1]), "\n")
    print("Center of mass: ", center_of_mass)



    # Write a small PyMol script to visualize the best line
    pdb.write_pml_script(best_line[0] + center_of_mass)
